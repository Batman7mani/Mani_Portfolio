...
export function ProjectDetail({ slug }: { slug: string }) {
  ...
  return <main className="project-detail"><header className="site-header"><Link href="/" className="wordmark">M/MC<span>.</span></Link><Link href="/" className="back-link">← Back to index</Link></header><section className={`detail-hero ${project.color} section-pad`}><div className="detail-visual"><img src={project.image} alt={project.imageAlt} /></div><div className="detail-hero-copy"><p className="eyebrow">Project {project.index} / {project.eyebrow}</p><h1 className="cursor-highlight">{project.title}</h1><p className="detail-summary">{project.summary}</p><div className="detail-shape" /></div></section><section className="detail-content section-pad"><div><p className="eyebrow">The idea</p><p className="detail-description cursor-highlight">{project.description}</p><p className="detail-kicker">A project built to turn a complicated brief into a clear, memorable moment.</p></div><div className="detail-side"><p className="eyebrow">Built with</p><ul>{project.stack.map((item) => <li key={item}>{item}</li>)}</ul><p className="eyebrow metrics-label">Impact</p>{project.metrics.map((metric) => <strong key={metric}>{metric}</strong>)}</div></section><section className="detail-story section-pad"><div className="story-heading"><p className="eyebrow">The making</p><h2>Good work does not just <span className="story-word">function</span>.<br />It leaves a <span className="story-word">feeling</span>.</h2></div><div className="story-beats"><article><span className="story-number">01</span><h3>Find the signal</h3><p>Start with the human problem, then remove everything that distracts from it. The result is a sharper product with a point of view.</p></article><article><span className="story-number">02</span><h3>Build the rhythm</h3><p>Structure, motion, and feedback work together so every interaction feels intentional, not merely assembled.</p></article><article><span className="story-number">03</span><h3>Make it matter</h3><p>The final measure is momentum: whether the experience helps people understand faster, choose with confidence, and come back.</p></article></div></section><footer className="detail-footer section-pad"><Link href="/">← All projects</Link><div className="detail-footer-actions"><a href={project.url} target="_blank" rel="noreferrer">Open project site <ArrowUpRight size={18} /></a><a href={profile.github} target="_blank" rel="noreferrer">More on GitHub <ArrowUpRight size={18} /></a></div></footer></main>
}
