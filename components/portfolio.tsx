...
export function ProjectDetail({ slug }: { slug: string }) {
  ...
  return <main className="project-detail"><header className="site-header"><Link href="/" className="wordmark">M/MC<span>.</span></Link><Link href="/" className="back-link">← Back to index</Link></header><section className={`detail-hero ${project.color} section-pad`}><div className="detail-visual"><img src={project.image} alt={project.imageAlt} /></div><div className="detail-hero-copy"><p className="eyebrow">Project {project.index} / {project.eyebrow}</p><h1 className="cursor-highlight">{project.title}</h1><p className="detail-summary">{project.summary}</p><div className="detail-shape" /></div></section><section className="detail-intro section-pad"><p className="eyebrow">A closer look</p><p className="detail-lede">Every project starts with a <span>specific tension</span> — then turns it into something people can feel, use, and remember.</p></section><section className="detail-idea section-pad"><p className="eyebrow">The idea</p><p className="detail-description cursor-highlight">{project.description}</p></section><section className="detail-explorer section-pad"><div className="detail-tabs" role="tablist" aria-label="Project details">{(['overview', 'stack', 'process'] as const).map((tab) => <button key={tab} role="tab" aria-selected={detailTab === tab} className={detailTab === tab ? 'is-active' : ''} onClick={() => setDetailTab(tab)}>{tab}</button>)}</div>{detailTab === 'overview' && <div className="detail-content"><div className="detail-built"><p className="eyebrow">Built for</p><p className="detail-audience">{project.audience ?? 'People who value clear, thoughtful digital experiences.'}</p></div><div className="detail-side"><p className="eyebrow metrics-label">Impact</p>{project.metrics.map((metric) => <strong key={metric}>{metric}</strong>)}</div></div>}{detailTab === 'stack' && <div className="detail-stack"><div className="detail-ring"><span>{languages.reduce((total, language) => total + Number.parseInt(language.lines), 0)}<small> lines of code</small></span></div><div className="detail-language-list">{languages.map((language) => <div className={`detail-language-row ${activeLanguage === language.name ? 'is-active' : ''}`} key={language.name} onMouseEnter={() => setActiveLanguage(language.name)} onMouseLeave={() => setActiveLanguage(null)}><i style={{ background: language.color }} /><strong>{language.name}</strong><span>{language.lines}</span><b>{language.percent}%</b><em style={{ width: `${language.percent}%`, background: language.color }} /></div>)}</div></div>}{detailTab === 'process' && <div className="detail-timeline">{process.map((item) => <article key={item.step}><span>{item.step}</span><div><h3>{item.title}</h3><p>{item.detail}</p></div></article>)}</div>}</section><section className="detail-proof section-pad"><p className="eyebrow">What mattered</p><div className="detail-proof-grid">{project.metrics.map((metric, index) => <div className="proof-item" key={metric}><span>0{index + 1}</span><strong>{metric}</strong><p>{index === 0 ? 'A clear path through the product, without asking the audience to work for it.' : index === 1 ? 'The interface gives the central idea room to become useful, memorable, and immediate.' : 'Designed to leave a measurable impression long after the first interaction.'}</p></div>)}</div></section><footer className="detail-footer section-pad"><Link href="/">← All projects</Link><div className="detail-footer-actions"><a href={project.url} target="_blank" rel="noreferrer">Open project site <ArrowUpRight size={18} /></a><a href={profile.github} target="_blank" rel="noreferrer">More on GitHub <ArrowUpRight size={18} /></a></div></footer></main>
}
